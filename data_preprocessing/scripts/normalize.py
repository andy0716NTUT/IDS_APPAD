...
def main() -> None:
	parser = argparse.ArgumentParser(description="Normalize dataset with MinMaxScaler using training set only.")
	parser.add_argument("--input", type=str, default=str(resolve_default_input()), help="Input CSV path")
	parser.add_argument("--output-dir", type=str, default=str(Path(__file__).resolve().parents[1] / "output" / "normalize"))
	parser.add_argument("--train-ratio", type=float, default=0.7)
	parser.add_argument("--val-ratio", type=float, default=0.15)
	parser.add_argument("--seed", type=int, default=42)
	parser.add_argument("--columns", type=str, default=None, help="Comma-separated numeric columns to normalize")
	...
	train_norm = apply_scaler(train_df)
	val_norm = apply_scaler(val_df)
	test_norm = apply_scaler(test_df)

	output_dir = Path(args.output_dir)
	output_dir.mkdir(parents=True, exist_ok=True)

	train_path = output_dir / "train_normalized.csv"
	val_path = output_dir / "val_normalized.csv"
	test_path = output_dir / "test_normalized.csv"

	train_norm.to_csv(train_path, index=False)
	val_norm.to_csv(val_path, index=False)
	...
	with (output_dir / "minmax_params.json").open("w", encoding="utf-8") as f:
